...
player_proj %>% select(Name, Pos, Team, Games.Played, Average.DK.Points)
...
merged_def = merged_def %>% select(lastname.x, firstname.x, Team, Pos.x, Salary, Proj_Points, Average.DK.Points)
...
merged = merged %>% select(lastname, firstname, Team.x, Pos, Salary, Proj_Points, Average.DK.Points)


rbind(merged, merged_def)
